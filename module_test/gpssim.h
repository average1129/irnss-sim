...
typedef struct
{
	...
	int dtls,tot,wnt;
	int dtlsf,dn,wnlsf;
} ionoutc_t; // what do these parameters mean? Read up - also read up for the ephemeris parameters - familiarity will help
...
typedef struct
{
	...
	double azel[2];
	double iono_delay;
} range_t; // read theory just for funsies ig? -- DONE 
