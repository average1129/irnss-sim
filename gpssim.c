...
// Why exactly do we need this reciever antenna attentuation pattern?  
...
{	
	//what is this delay matrix, why exactly is this created?
	int delay[] = {
	...
	{	
		...
		for (j=9; j>0; j--) 
		{
			r1[j] = r1[j-1]; // shift register code ? 
			r2[j] = r2[j-1]; //  
		}
		r1[0] = c1;
		r2[0] = c2;
	...
	// create cout prints for this function 
	return;
}
...
{
	// this llh to ecef is the clue here ? why is this transformation importatn? where is it used?
	double slat, clat;
	double slon, clon;
...
/*! \brief Convert Earth-centered Earth-Fixed to ? // V SUS -> what doe neu mean? 
...
{ printf("line 1260 computeRange  function called : \n");
			...
			if (allocatedSat[sv]==-1) // Visible but not allocated
			{
				...
				for (i=0; i<MAX_CHAN; i++)
				{
					if (chan[i].prn==0)
					{
						...
						computeRange(&rho, eph[sv], &ionoutc, grx, ref); // this will give range from 0,0 in ecef ? then it means range from centre pf eaerth 
						//printf("allocateChannel -> Compute Range Function called : line 1667");
						r_ref = rho.range; // computeRange used twice in a row -> 
...
int main(int argc, char *argv[])
{
	...
	int sv;
	int neph,ieph;
	ephem_t eph[EPHEM_ARRAY_SIZE][MAX_SAT]; // lets dump this variable 
	gpstime_t g0;
	...
	char outfile[MAX_CHAR]; // how this outfile populated?
	...
	printf("iduration = %d ", iduration);
	duration = (double)iduration/10.0; // Default duration
	printf("dduration = %f ", duration);
	verb = FALSE;
	ionoutc.enable = TRUE;
	...
	printf("iq_buff_size = %d", iq_buff_size);
	samp_freq *= 10.0;
	printf("samp_freq = %f", samp_freq);
	delt = 1.0/samp_freq;
	printf("delt= %f", delt);
	////////////////////////////////////////////////////////////
	// Receiver position
	...
	{ 
		...
		// Set simulation duration
		numd = iduration;
		printf("Simulation Duration: %d", numd);
		// Set user initial position
		llh2xyz(llh, xyz[0]);
	}
	...
	neph = readRinexNavAll(eph, &ionoutc, navfile); //populate this function 
	...
	printf("readRinexNavAll called");
	printf ("Number of sets of ephemerides in the file: %d", neph);
	//we can rpint out what all ephemerides are being loaded 
	...
	{	printf("%d \n", sv);
		...
		{	
			gmin = eph[0][sv].toc;
			printf("gmin = eph[0][sv = %d].toc.sec = %f", sv, eph[0][sv].toc.sec);
			tmin = eph[0][sv].t;
			printf("tmin = eph[0][sv = %d].t.sec =%f",sv, eph[0][sv].t.sec);
			break;
		}
	}
	...
	for (sv=0; sv<MAX_SAT; sv++)
	{
		...
		{	
			...
			printf("gmax = eph[neph-1][sv = %d].toc.sec = %f \n", sv, eph[neph-1][sv].toc.sec);
			tmax = eph[neph-1][sv].t;
			printf("tmax = eph[neph-1][sv = %d].t.sec =%f \n",sv, eph[neph-1][sv].t.sec);
			break;
		}
	}

	// make a print out of ephemeris
	// i think g0 will be -1 here - only initialised to -1 at beginning, no changes 
	printf ("g0 = %d weeks %f secs \n",g0.week, g0.sec);
	
	if (g0.week>=0) // Scenario start time has been set through cli
	{ printf ("Scenario start time has been set :  g0.week =  %d, g0.sec = %f\n", g0.week,g0.sec);
		if (timeoverwrite==TRUE)
		{
			...
			for (sv=0; sv<MAX_SAT; sv++)
			{
				...
				{
					if (eph[i][sv].vflg == 1)
					{	printf ("eph[i][sv].vflg == 1 - need to add more debug statements at ln 2079\n");
	...
	printf("Selecting the current set of ephemerides\n");
	ieph = -1;
	printf("neph:%d \n", neph);
	...
	{	
		...
		{
			if (eph[i][sv].vflg == 1)
			{	printf ("Ephemeris No.: %d,Satellite No: %d \n", i, sv);
				dt = subGpsTime(g0, eph[i][sv].toc);
				//printf("dt[%f] == g0[%f]- Satellite_Timeof Clock(%d)[%d][%f]) \n",dt, g0, eph[i][sv].toc,sv);
				if (dt>=-SECONDS_IN_HOUR && dt<SECONDS_IN_HOUR)
				{	printf("dt is  between - SECONDS IN HOUR & SECONDS_INHOUR -3600< dt< 3600");
					ieph = i;
					printf("ieph= %d",i);
					break;
				}
...
// only single hit required to set ieph  = initial ephemeris 
	...
	{ // iterating through all file samples 
		for (i=0; i<MAX_CHAN; i++)
		{ //iterating through all channels
			if (chan[i].prn>0) // visible and allocated satellite 
			{
				...
				if (!staticLocationMode)
					...
					printf("non static location mode Compute Range call from main- line no 2253 \n");}
				else
					{computeRange(&rho, eph[ieph][sv], &ionoutc, grx, xyz[0]);
					printf(" static location mode Compute Range call from main - line no. 2258 \n");}
				...
				printf ("computeCodePhase - line 2263- update code phase and data bit counters \n");
